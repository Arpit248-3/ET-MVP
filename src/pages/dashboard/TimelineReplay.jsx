...
import { fetchTimeline, triggerDemoStep } from '../../services/api.js';
...
export default function TimelineReplay() {
  ...
  // Fetch live timeline
  const { data: timelineResponse, loading: timelineLoading, error: timelineError, refetch } = useApi(fetchTimeline, {
    fallback: null,
  });

  const liveEvents = timelineResponse?.events || [];
  const currentStep = timelineResponse?.current_step || 0;
  ...
  return (
    <DashboardLayout>
      <PageHeader title="Timeline Replay" subtitle="Replay historical energy crisis events with AI-annotated decision playback"
        badge={{ label: backendOnline ? 'LIVE SYNC' : 'DEMO MODE', color: backendOnline ? '#22c55e' : '#8b5cf6' }}
      />
      ...
      {!backendOnline ? (
        <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.2)', color: '#f59e0b', padding: '10px 16px', borderRadius: 8, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
          <WifiOff size={14} />
          Backend Offline. Timeline is displaying offline mock simulation curve.
        </div>
      ) : timelineLoading ? (
        <div style={{ background: 'rgba(29,140,255,0.1)', border: '1px solid rgba(29,140,255,0.2)', color: '#1d8cff', padding: '10px 16px', borderRadius: 8, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
          ...
          Aligning timeline index nodes from demo session...
        </div>
      ) : timelineError ? (
        <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#ef4444', padding: '10px 16px', borderRadius: 8, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
          ...
          Timeline Sync Error: {timelineError.message || 'Connection failed'}. Showing offline mockup.
        </div>
      ) : null}
      ...
      <GlassCard className="card" style={{ padding: '16px 20px', marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <div style={{ fontSize: 13, fontWeight: 600 }}>
            {timelineResponse?.scenario_name || activeScenario?.name || 'Baseline Operations'} – Timeline
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 14, padding: '10px 0', borderTop: '1px solid var(--border-soft)' }}>
          <button onClick={() => handleStepSelect(0)} disabled={!backendOnline} style={{ background: 'rgba(255,255,255,0.06)', border: 'none', borderRadius: 6, padding: '6px 10px', cursor: 'pointer', color: 'var(--text-secondary)' }}><SkipBack size={14} /></button>
          <button onClick={handlePrev} disabled={!backendOnline} style={{ background: 'rgba(255,255,255,0.06)', border: 'none', borderRadius: 6, padding: '6px 10px', cursor: 'pointer', color: 'var(--text-secondary)' }}>Prev</button>
          ...
          <button onClick={handleNext} disabled={!backendOnline} style={{ background: 'rgba(255,255,255,0.06)', border: 'none', borderRadius: 6, padding: '6px 10px', cursor: 'pointer', color: 'var(--text-secondary)' }}>Next</button>
          <button onClick={() => handleStepSelect(chartData.length - 1)} disabled={!backendOnline} style={{ background: 'rgba(255,255,255,0.06)', border: 'none', borderRadius: 6, padding: '6px 10px', cursor: 'pointer', color: 'var(--text-secondary)' }}><SkipForward size={14} /></button>
